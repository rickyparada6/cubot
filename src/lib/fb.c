 ...
 *
 * This program implements the framebuffer module for the RPi.
 */
#include "fb.h"
#include "mailbox.h"
...
void fb_init(unsigned int width, unsigned int height, unsigned int depth_in_bytes, fb_mode_t mode)
{
    ...
    else fb.virtual_height = height * 2;
...
{
    fb.y_offset = (fb.y_offset + fb.height) % fb.virtual_height;
    mailbox_write(MAILBOX_FRAMEBUFFER, (unsigned int)&fb);
    mailbox_read(MAILBOX_FRAMEBUFFER);
}
...
{
    if (fb.height == fb.virtual_height || fb.y_offset != 0) return (void *)fb.framebuffer;
    else return (void *)(fb.framebuffer + (fb_get_pitch() * fb_get_height()));
}
